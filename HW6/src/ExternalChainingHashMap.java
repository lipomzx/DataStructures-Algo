 ...
 * @GTID 903649317
 *
 * Collaborators: Adavya
 *
 * Resources: https://www.javamadesoeasy.com/2015/02/hashmap-custom-implementation.html
 * https://stackoverflow.com/questions/54049966/should-i-throw-an-exception-or-return-false
 */
public class ExternalChainingHashMap<K, V> {
     ...
     *
     * @param capacity the initial cgit staapacity of the backing array
     */
    ...
    public V put(K key, V value) {
        ...
        if (table[index] == null) {
            ...
            ExternalChainingMapEntry<K, V> existEntry = table[index];
            while (existEntry != null) {
                if (existEntry.getKey().equals(key)) {
                    V oldV = existEntry.getValue();
                    existEntry.setValue(value);
                    return oldV;
                }
                existEntry = existEntry.getNext();
            }
            ExternalChainingMapEntry<K, V> newE = new ExternalChainingMapEntry<>(key, value);
            newE.setNext(table[index]);
            table[index] = newE;
        }
        size++;
        return null;
    ...
    public V get(K key) {
        ...
        ExternalChainingMapEntry<K, V> entry = table[position];
        while (entry != null) {
    ...
    public Set<K> keySet() {
        ...
        for (ExternalChainingMapEntry<K, V> entry : table) {
            ...
            if (count == size) // stop for loop when all elements have been traversed
                break;
        }
        return keys;
    }
     ...
     */
    public void clear() {
        this.table = new ExternalChainingMapEntry[INITIAL_CAPACITY];
        this.size = 0;
    }
